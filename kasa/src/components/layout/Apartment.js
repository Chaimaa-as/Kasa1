...
const Apartment = (props) => {
  ...
  return (
    ...
    <NavLink to={`/logement/${props.item.id}`} className="apartments">
      {/*Ce div est un conteneur pour les éléments suivants.
      Il a une classe CSS "cards" qui lui est attribuée. */}
      <div className="cards">
        {/* {props.item.cover} : Cette balise img affiche l'image de couverture du logement. 
        L'attribut "src" est défini avec la valeur de "props.item.cover", qui est l'URL de l'image. */}
        <img src={props.item.cover} alt="" />

        {/* {props.item.title} : Cette balise h2 affiche le titre du logement. 
